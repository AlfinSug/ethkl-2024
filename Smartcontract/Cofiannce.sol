...
// File: contracts/Interface/ICollateralLockPool.sol
...
interface ICollateralLockPool {
    ...
    function lockedCollateralA(address user) external view returns (uint256);
    function lockedCollateralB(address user) external view returns (uint256);
}
...
contract CoFinance is ReentrancyGuard {
    ...
    uint256 public totalSwapFeesA; // Total fees accumulated for token A
    uint256 public totalSwapFeesB; // Total fees accumulated for token B
    uint public estimatedTokenAmount;
    ...
    function dynamicSwap(
        ...
        if (tokenAddress == address(tokenA)) {
            return (amountAfterFee * priceTokenA) / priceTokenB; 
        } else {
            return (amountAfterFee * priceTokenB) / priceTokenA; 
        }
    }

    function constantSwap(
        address tokenAddress,
        uint256 amountAfterFee
     ) internal view returns (uint256) {
        uint256 netReserveA = tokenA.balanceOf(_thisAddress);
        uint256 netReserveB = tokenB.balanceOf(_thisAddress);
        
        if (tokenAddress == address(tokenA)) {
            return (amountAfterFee * netReserveB) / (netReserveA + amountAfterFee); // A to B
        } else {
            return (amountAfterFee * netReserveA) / (netReserveB + amountAfterFee); // B to A
        }
    }

    function previewSwap(address tokenAddress, uint256 tokenAmount) external view returns (uint256 estimatedAmount) {
        ...
        if (tokenAddress == address(tokenA)) {
            if (priceTokenA != 0 && priceTokenB != 0) {
                estimatedAmount = dynamicSwap(tokenAddress, amountAfterFee, priceTokenA, priceTokenB);
            } else {
                estimatedAmount = constantSwap(tokenAddress, amountAfterFee);
            }
        } else if (tokenAddress == address(tokenB)) {
            if (priceTokenA != 0 && priceTokenB != 0) {
                estimatedAmount = dynamicSwap(tokenAddress, amountAfterFee, priceTokenB, priceTokenA);
            } else {
                estimatedAmount = constantSwap(tokenAddress, amountAfterFee);
            }
        } else {
    ...
    function swapTokens(address tokenAddress, uint256 tokenAmount) external {
        ...
        uint256 priceTokenA = priceFeed.getTokenAPrice();
        uint256 priceTokenB = priceFeed.getTokenBPrice();
        require(priceTokenA != 0 && priceTokenB != 0, "Invalid price feed values");
        uint256 tokenBAmount;
        uint256 tokenAAmount;
        if (tokenAddress == address(tokenA)) {
            tokenBAmount = priceTokenA != 0 && priceTokenB != 0
                ? dynamicSwap(tokenAddress, amountAfterFee, priceTokenA, priceTokenB)
                : constantSwap(tokenAddress, amountAfterFee);
            ...
            tokenAAmount = priceTokenA != 0 && priceTokenB != 0
                ? dynamicSwap(tokenAddress, amountAfterFee, priceTokenB, priceTokenA)
                : constantSwap(tokenAddress, amountAfterFee);
    ...
    return tokenA.balanceOf(_thisAddress);
    }
    ...
    function borrowTokens(address tokenToBorrow, uint256 amount, uint256 duration) external hasCollateral {
        ...
        require(tokenToBorrow == address(tokenA) || tokenToBorrow == address(tokenB), "Invalid token to borrow");
        
        // Fetch prices
        uint256 priceA = priceFeed.getTokenAPrice();
        uint256 priceB = priceFeed.getTokenBPrice();
        require(priceA != 0 && priceB != 0, "Price feed error");
        uint256 interest = calculateInterest(amount, block.timestamp, duration);
        uint256 totalAmountOwed = amount + interest; 
        uint256 collateralValueInUSD;
        if (tokenToBorrow == address(tokenA)) {
            collateralValueInUSD = (collateralB[msg.sender] * priceB);
            require(collateralValueInUSD >= (totalAmountOwed * priceA * 20) / 100, "Collateral must exceed 20% of total borrowed amount with interest");
        } else {
            collateralValueInUSD = (collateralA[msg.sender] * priceA);
            require(collateralValueInUSD >= (totalAmountOwed * priceB * 20) / 100, "Collateral must exceed 20% of total borrowed amount with interest");
        }

        require(totalAmountOwed <= collateralValueInUSD * 80 / 100, "Total borrowed amount exceeds 80% of collateral");

        if (tokenToBorrow == address(tokenA)) {
            collateralB[msg.sender] -= (amount * 100) / priceB; 
        } else {
            collateralA[msg.sender] -= (amount * 100) / priceA;
        }
        borrowed[msg.sender] += totalAmountOwed;
        loanStartTime[msg.sender] = block.timestamp;
        loanDuration[msg.sender] = duration;
        borrowedToken[msg.sender] = tokenToBorrow; 
        IERC20(tokenToBorrow).safeTransfer(msg.sender, amount);

        emit TokensBorrowed(msg.sender, amount, tokenToBorrow, duration);
    }




    function repayLoan(uint256 amount, address tokenAddress) external {
        require(amount != 0, "Amount must be greater than 0");
        require(borrowed[msg.sender] > 0, "No loan to repay");
        require(borrowedToken[msg.sender] == tokenAddress, "Incorrect token address");

        uint256 totalLoanAmount = borrowed[msg.sender];
        uint256 interest = calculateInterest(totalLoanAmount, loanStartTime[msg.sender], loanDuration[msg.sender]);
        uint256 totalRepayment = totalLoanAmount + interest;
        require(amount <= totalRepayment, "Repayment exceeds total amount including interest");
        IERC20(tokenAddress).safeTransferFrom(msg.sender, address(this), amount);
        if (amount >= totalLoanAmount) {
            borrowed[msg.sender] = 0; 
            loanStartTime[msg.sender] = 0;
            loanDuration[msg.sender] = 0;
            borrowedToken[msg.sender] = address(0); 
        } else {
    ...
    uint256 netReserveA = IERC20(tokenA).balanceOf(_thisAddress);
    uint256 netReserveB = IERC20(tokenB).balanceOf(_thisAddress);
    ...
    function withdrawLiquidity(uint256 liquidityTokenAmount) external {
        ...
        uint256 netReserveA = IERC20(tokenA).balanceOf(_thisAddress) - totalSwapFeesA - collateralA[_thisAddress];
        uint256 netReserveB = IERC20(tokenB).balanceOf(_thisAddress) - totalSwapFeesB - collateralB[_thisAddress];
    ...
    function getBorrowedInfo(address user) external view returns (address tokenAddress, uint256 amount) {
        tokenAddress = borrowedToken[user];
        amount = borrowed[user];
    }

    function getTotalLiquidity() external view returns (uint256 netReserveA, uint256 netReserveB) {
        netReserveA = IERC20(tokenA).balanceOf(_thisAddress) - totalSwapFeesA - collateralA[_thisAddress];
        netReserveB = IERC20(tokenB).balanceOf(_thisAddress) - totalSwapFeesB - collateralB[_thisAddress];
        if (netReserveA < 0) {
            netReserveA = 0;
        }
        if (netReserveB < 0) {
...
// File: contracts/CollateralLockPool.sol
